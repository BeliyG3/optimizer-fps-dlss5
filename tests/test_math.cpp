...
{
    const ofps::sdk::ConfigV1 config = ofps::sdk::DefaultConfigV1();
    ofps::sdk::LayoutV1 layout{};
    Check(ofps::sdk::BuildLayout(config, 3840, 2160, &layout) == ofps::sdk::Status::Ok, "default layout builds");
...
{
    ofps::sdk::LayoutV1 layout{};
    Check(ofps::sdk::BuildLayout(ofps::sdk::DefaultConfigV1(), 3840, 2160, &layout) == ofps::sdk::Status::Ok, "round-trip layout builds");
    float maxError = 0.0f;
    for (int y = -128; y <= 2288; y += 17) {
        for (int x = -128; x <= 3968; x += 19) {
            const ofps::sdk::Float2 p{static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f};
            const ofps::sdk::Float2 q = ofps::sdk::PackPosition(p, layout);
            const ofps::sdk::Float2 r = ofps::sdk::UnpackPosition(q, layout);
...
void TestCurveProperties()
{
    ...
    const float derivativeAtCenter =
        (ofps::sdk::PackRadius(center + h, center, work) - ofps::sdk::PackRadius(center - h, center, work)) / (2.0f * h);
    const float derivativeBeforeEdge =
        (ofps::sdk::PackRadius(1.0f, center, work) - ofps::sdk::PackRadius(1.0f - h, center, work)) / h;
    const float derivativeAfterEdge =
        (ofps::sdk::PackRadius(1.0f + h, center, work) - ofps::sdk::PackRadius(1.0f, center, work)) / h;
    ...
    ofps::sdk::LayoutV1 layout{};
    Check(ofps::sdk::BuildLayout(ofps::sdk::DefaultConfigV1(), 3840, 2160, &layout) == ofps::sdk::Status::Ok,
    ...
    for (std::uint32_t axis = 0; axis != 2; ++axis) {
        ...
        float previous = ofps::sdk::PackCoordinate(-64.0f, axis, layout);
        for (float value = -63.75f; value <= nativeExtent + 64.0f; value += 0.25f) {
            const float current = ofps::sdk::PackCoordinate(value, axis, layout);
            Check(current > previous, "packed coordinate is strictly monotonic");
            previous = current;
        }
        for (float value = -32.0f; value <= nativeExtent + 32.0f; value += 7.25f) {
            const float left = ofps::sdk::PackCoordinate(value, axis, layout);
            const float right = ofps::sdk::PackCoordinate(nativeExtent - value, axis, layout);
...
{
    ofps::sdk::LayoutV1 layout{};
    Check(ofps::sdk::BuildLayout(ofps::sdk::DefaultConfigV1(), 3840, 2160, &layout) == ofps::sdk::Status::Ok, "motion layout builds");
    const ofps::sdk::Float2 positions[] = {{1920.5f, 1080.5f}, {200.5f, 100.5f}, {3500.5f, 2000.5f}};
    const ofps::sdk::Float2 motions[] = {{12.25f, -7.5f}, {-80.0f, 24.0f}, {500.0f, -300.0f}};
    for (const auto &position : positions) {
        const ofps::sdk::Float2 packedPosition = ofps::sdk::PackPosition(position, layout);
        for (const auto &motion : motions) {
            const ofps::sdk::Float2 packedMotion = ofps::sdk::PackMotion(position, motion, layout);
...
void TestIdentityAndUniform()
{
    ...
    ofps::sdk::LayoutV1 off{};
    Check(ofps::sdk::BuildLayout(config, 3840, 2160, &off) == ofps::sdk::Status::Ok, "off layout builds");
    Check(off.workWidth == 3840 && Near(ofps::sdk::PackCoordinate(101.25f, 0, off), 101.25f), "off is identity");

    config.mode = ofps::sdk::WarpMode::Uniform;
    config.xAxis.workPercent = 90.0f;
    config.yAxis.workPercent = 90.0f;
    ofps::sdk::LayoutV1 uniform{};
    Check(ofps::sdk::BuildLayout(config, 3840, 2160, &uniform) == ofps::sdk::Status::Ok, "uniform layout builds");
    Check(Near(ofps::sdk::PackCoordinate(1920.0f, 0, uniform), 1728.0f), "uniform scales coordinates");
}
...
void TestValidation()
{
    ...
    Check(ofps::sdk::ValidateConfig(config) == ofps::sdk::Status::InvalidAxis, "compression guard rejects an edge slope below one quarter");
    config = ofps::sdk::DefaultConfigV1();
    ...
    config = ofps::sdk::DefaultConfigV1();
    config.structSize = 0;
    Check(ofps::sdk::ValidateConfig(config) == ofps::sdk::Status::StructSizeMismatch, "structure size is enforced");
    config = ofps::sdk::DefaultConfigV1();
    ofps::sdk::LayoutV1 layout{};
    Check(ofps::sdk::BuildLayout(config, 3839, 2160, &layout) == ofps::sdk::Status::Ok && (layout.workWidth & 1u) == 0,
    ...
    Check(ofps::sdk::ValidateLayout(layout) == ofps::sdk::Status::Ok, "a canonical layout validates");
    layout.edgeSlopeX = 0.75f;
    Check(ofps::sdk::ValidateLayout(layout) == ofps::sdk::Status::InvalidAxis,
          "layout validation rejects inconsistent derived values");

    config = ofps::sdk::DefaultConfigV1();
    config.reserved[0] = 1;
    Check(ofps::sdk::ValidateConfig(config) == ofps::sdk::Status::InvalidFlags,
...
{
    ofps::sdk::ConfigV1 config = ofps::sdk::DefaultConfigV1();
    config.flags &= ~ofps::sdk::ConfigFlagExtendMotionAtEdge;
    ofps::sdk::LayoutV1 clamped{};
    Check(ofps::sdk::BuildLayout(config, 320, 180, &clamped) == ofps::sdk::Status::Ok,
          "clamped-motion layout builds");

    const ofps::sdk::Float2 current{0.5f, 90.5f};
    const ofps::sdk::Float2 outsideMotion{-40.0f, 0.0f};
    const ofps::sdk::Float2 clampedMotion = ofps::sdk::PackMotion(current, outsideMotion, clamped);
    Check(Near(clampedMotion.x, 0.0f), "cleared edge flag clamps motion endpoints to the first pixel center");

    config.flags |= ofps::sdk::ConfigFlagExtendMotionAtEdge;
    ofps::sdk::LayoutV1 extended{};
    Check(ofps::sdk::BuildLayout(config, 320, 180, &extended) == ofps::sdk::Status::Ok,
          "extended-motion layout builds");
    const ofps::sdk::Float2 extendedMotion = ofps::sdk::PackMotion(current, outsideMotion, extended);
...
int main()
{
    ...
    std::cout << "Optimizer FPS SDK CPU tests passed\n";
    return EXIT_SUCCESS;
}
