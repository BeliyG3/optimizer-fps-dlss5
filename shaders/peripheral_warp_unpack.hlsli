#ifndef PERIPHERAL_WARP_UNPACK_HLSLI
#define PERIPHERAL_WARP_UNPACK_HLSLI

// The Unpack stage as a reusable include, like peripheral_warp_pack.hlsli: the four packed inputs and
// PwUnpackTexel, which produces one native texel from a native pixel position. unpack.hlsl wraps it in
// a full-screen pixel shader; warp_unpack_cs.hlsl wraps it in a compute shader for hosts that record
// the model into a compute command list.
//
// Requires peripheral_warp_common.hlsli.
...
void PwUnpackTexel(float2 nativePixel, out float4 color, out float depth, out float2 motion, out float confidence)
{
    ...
    color = PwPackedColor.SampleLevel(PwLinearClamp, workUv, 0.0);
    ...
    depth = PwPackedDepth.Load(int3(guidePixel, 0));
    motion = PwUnpackMotionPixels(guideCenter, packedMotion);
    confidence = PwPackedConfidence.Load(int3(guidePixel, 0));
