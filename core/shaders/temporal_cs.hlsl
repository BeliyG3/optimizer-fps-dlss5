...
float3 PwTLimitToFrame(float3 value, uint2 id)
{
    ...
    if (PwTSmooth.z <= 0.0 || PwTSmooth.x < 0.0) return value;
...
{
    if (PwTOutside(id.xy)) return;
    const uint gridWidth = PwTTargetSize.x / 4;
    const uint tile = id.x / gridWidth;
    const float2 uv = (float2(id.x % gridWidth, id.y) + 0.5) / float2(gridWidth, PwTTargetSize.y);
    const float2 texel = PwTMotionRect.xy + uv * PwTMotionRect.zw;
    const float2 p = floor(uv * PwTNative.xy) + 0.5;
    if (tile != 0)
    {
        float dc = PwTDepth.Load(int3(int2(PwTToRect(p, PwTDepthRect)), 0));
        float2 direct = PwTAccPrev.Load(int3(int2(texel), 0));
        float2 fetched = PwTFetchAcc(p, dc);
        float2 q = p + fetched * PwTNative.xy / PwTMotionRect.zw;
        float2 raw = PwTMotion.Load(int3(int2(texel), 0));
        float df = PwTDepthF.Load(int3(int2(PwTToRect(p, PwTDepthRect)), 0));
        float edge = saturate(min(min(q.x, PwTNative.x-q.x), min(q.y, PwTNative.y-q.y))/16.0);
        PwTOut0[id.xy] = tile == 1 ? float4(raw, dc, df) : tile == 2 ? float4(direct, fetched) :
            float4(edge, PwTDepthMismatchAround(PwTExpectedDepth(dc, p), q), PwTExpectedDepth(dc, p),
                length(raw * PwTMotionTex.zw * PwTNative.xy / PwTMotionRect.zw));
        return;
    }
    float2 acc = PwTAccPrev.Load(int3(int2(texel), 0));
    if (any(!isfinite(acc))) acc = float2(0.0, 0.0);
    float3 health = float3(-1.0, 0.0, 0.0);
    if (PwTParams.z == -1.0)
    {
        PwFullscreenVertex v = PwTVertex(id.xy);
        v.position.xy = floor(uv * PwTNative.xy) + 0.5;
        float accepted;
        PwTReproject(v, accepted, health);
    }
    PwTOut0[id.xy] = float4(length(acc * PwTNative.xy / PwTMotionRect.zw), health);
}
