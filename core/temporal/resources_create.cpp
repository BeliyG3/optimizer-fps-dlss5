...
bool Resources::Create(ID3D12Device *dev, const ofps::core::gpu::Shaders &shaders, std::uint32_t nativeWidth,
                       ...
                       std::uint32_t pictureDivisor)
{
    ...
    historyPictureDivisor = pictureDivisor;
    ...
    std::string computeReason;
    if (!gpu::CreateComputeRoot(device, rs, &rootSignature, computeReason))
        return fail("root signature serialisation failed");

#define PW_TEMPORAL_PASS(name, member, reads, outputs, extent) \
    if (!gpu::CreateComputePass(device, rootSignature, shaders.temporal##name.data(), \
                                shaders.temporal##name.size(), #name, &member##Pso, computeReason)) \
        return fail("compute pipeline " #name " missing or unsupported");
#include "core/shaders/temporal_passes.def"
#undef PW_TEMPORAL_PASS
    if (!gpu::CreateComputePass(device, rootSignature, shaders.temporalRefineModel.data(),
                                shaders.temporalRefineModel.size(), "RefineModel", &refineModelPso, computeReason) ||
    ...
    hd.NumDescriptors = (kTableSlots + kFallbackTableSlots) * kTableSize;
    ...
    descriptors.Reset(kTableSlots);
    fallbackDescriptors.Reset(kFallbackTableSlots);
    ...
    if (downsamplePso) {
        ...
        if (!ofps::core::gpu::CreateTexture(device, lowW, lowH, kResidualFormat, kTarget, &residualLow) ||
            !ofps::core::gpu::CreateTexture(device, nativeWidth, nativeHeight, kResidualFormat, kTarget, &toneAcc))
    ...
    if (modelMotionPso && !ofps::core::gpu::CreateTexture(device, motionWidth, motionHeight, kChainFormat, kTarget, &modelMv))
