...
{
    pictureW_ = (width + pictureDivisor - 1) / pictureDivisor;
    pictureH_ = (height + pictureDivisor - 1) / pictureDivisor;
    geometryW_ = (width + 5) / 6; geometryH_ = (height + 5) / 6;
    for (auto &pass : passes_)
        for (unsigned i = 0; i < 4; ++i)
            if (!ofps::core::gpu::CreateTexture(device, i < 2 ? pictureW_ : geometryW_, i < 2 ? pictureH_ : geometryH_,
                    ...
                    const SlotKey &previous, bool connected)
{
    ...
    for (unsigned i = 0; i < 4; ++i)
        ofps::core::gpu::Barrier(cmd, pass.textures[i], pass.states[i], D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    ...
    for (unsigned i = 0; i < 4; ++i)
        ofps::core::gpu::Barrier(cmd, pass.textures[i], pass.states[i], kReadable);
    newest_ = slot;
    count_ = (std::min)(count_ + 1, 2u);
}

void History::Bind(Resources &, ID3D12GraphicsCommandList *cmd, SlotKey &key, Constants &constants,
                   std::uint32_t maxPasses)
{
    const auto count = (std::min)(count_, maxPasses);
    constants.params[0] = static_cast<float>(count);
