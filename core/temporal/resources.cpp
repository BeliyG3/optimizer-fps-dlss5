...
D3D12_GPU_DESCRIPTOR_HANDLE Resources::Table(const SlotKey &key, Output output, const Output *second, bool fallback)
{
    if (exhausted) return {};
    auto &pool = fallback ? fallbackDescriptors : descriptors;
    const auto acquired = pool.Acquire(usePoint, evalNow, gpu::kPoolWaitMilliseconds);
    if (acquired == gpu::DescriptorPool::kNone) { exhausted = true; return {}; }
    const auto slot = fallback ? kTableSlots + acquired : acquired;
...
void Resources::Dispatch(ID3D12GraphicsCommandList *cmd, Pass pass, Output output,
                         ...
                         const Output *second, FlowExtent flow)
{
...
#undef PW_TEMPORAL_PASS
    ...
    SlotKey bound = key;
    const unsigned reads = passes[pass].reads | ((pass == kFlowLuma && constants.hostModelGrid) ? (1u << 5) : 0u);
    for (unsigned i = 0; i < kTableUsed; ++i)
        if ((reads & (1u << i)) == 0) bound.res[i] = nullptr;
    const auto table = Table(bound, output, passes[pass].outputs > 1 ? second : nullptr,
    ...
    timer.End(cmd, usePoint, timingSlot);
...
FrameInputs Resources::Resolve(const FrameInputs &raw) const
{
    ...
    if (raw.base) {
        ...
        in.colorSubresource = raw.baseSubresource;
    } else {
...
Constants Resources::BaseConstants(const FrameInputs &in) const
{
    ...
    c.tune[2] = in.rawInterpolation ? 1.0f : (in.historySearch ? 0.0f : -1.0f);
    ...
    c.smoothing[0] = in.lumaBand ? 0.0f : -1.0f;
    c.smoothing[1] = in.smoothRadius;
    c.smoothing[2] = in.ratioScale;
    c.smoothing[3] = phase.Share();
    c.hostModelGrid = in.hostModelGrid ? 1u : 0u;
    return c;
}
