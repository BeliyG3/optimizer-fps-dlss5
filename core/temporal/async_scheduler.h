...
struct AsyncJob final : public ofps::core::gpu::Disposable {
    ...
    // UI and codec resources belong to the same fenced job as the primary inputs.
    ID3D12Resource *extraBg[7] = {};
    D3D12_RESOURCE_STATES extraState[7] = {};
    OfpsFrameInputs frameSnapshot{};
    OfpsResource codecBefore[2] = {};
    ID3D12Resource *codecBase[2] = {};
    D3D12_RESOURCE_STATES codecBaseState[2] = {};
    bool codecIdentity = true;
    ...
    bool inflight = false;
    bool recording = false; // retirement also protects a job currently being recorded
    ...
    DXGI_FORMAT bgPackColorView = DXGI_FORMAT_UNKNOWN, bgPackDepthView = DXGI_FORMAT_UNKNOWN, bgPackMotionView = DXGI_FORMAT_UNKNOWN;
    ...
    void Settle()
    {
        ...
        if (signalPending && fInputs) {
            ...
            else ofps::core::gpu::SignalRegisteredQueues(device, nullptr, fInputs, jobId);
            signalPending = false;
        }
    ...
    {
        Settle();
        if (AsyncVerbose() && queue) ofps::core::Log(false, "Optimizer FPS NGX hook [async] releasing the background job objects");
        ...
        for (auto &r : codecBase) if (r) { r->Release(); r = nullptr; }
        for (auto &r : extraBg) if (r) { r->Release(); r = nullptr; }
        ...
        hostSignalled = 0;
        if (submitQueue) { submitQueue->Release(); submitQueue = nullptr; }
