                         ...
                         DXGI_FORMAT depthFormat, std::uint32_t depthWidth, std::uint32_t depthHeight, char *error, std::size_t errorSize)
{
    ...
    if (!resources->Create(device, shaders, nativeWidth, nativeHeight, outputFormat, outputView, motionWidth, motionHeight,
                           depthFormat, depthWidth, depthHeight, error, errorSize)) {
        delete resources;
        return false;
    }
    res_ = resources;
                      ...
                      std::uint32_t motionHeight, DXGI_FORMAT depthFormat, std::uint32_t depthWidth, std::uint32_t depthHeight) const
{
    return res_ != nullptr && res_->nativeW == nativeWidth && res_->nativeH == nativeHeight && res_->outputFormat == outputFormat &&
           ...
           res_->depthW == depthWidth && res_->depthH == depthHeight;
}
                              ...
                              D3D12_RESOURCE_STATES hostOutputState, std::uint32_t dstX, std::uint32_t dstY)
{
    ...
    CopyInterp(cmd, hostOutput, hostOutputState, dstX, dstY);
}

void Machine::RecordRaw(ID3D12GraphicsCommandList *cmd, const FrameInputs &in, ID3D12Resource *target,
                        D3D12_RESOURCE_STATES state, std::uint32_t dstX, std::uint32_t dstY)
{
    ...
    raw.debugVis = 3;
    raw.phaseInFrames = 0;
    RecordReproject(cmd, raw, target, state, dstX, dstY);
    res_->phase = phase;
}
