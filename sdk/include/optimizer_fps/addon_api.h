...
using ConsumerCallbackV1 = void (OFPS_CALL *)(const PackedFrameV1 *frame, void *userData);
...
using RegisterConsumerFnV1 = Status (OFPS_CALL *)(const ConsumerRegistrationV1 *, std::uint64_t *);
using UnregisterConsumerFnV1 = Status (OFPS_CALL *)(std::uint64_t);
using GetConfigFnV1 = Status (OFPS_CALL *)(ConfigV1 *);
using SetConfigFnV1 = Status (OFPS_CALL *)(const ConfigV1 *);
using GetLayoutFnV1 = Status (OFPS_CALL *)(std::uint64_t, LayoutV1 *, std::uint64_t *);
using SubmitInputFnV1 = Status (OFPS_CALL *)(const FrameInputV1 *);
using PublishPackedFrameFnV1 = Status (OFPS_CALL *)(const PackedFrameV1 *);
...
#if defined(_WIN32)
#define OFPS_ADDON_EXPORT extern "C" __declspec(dllexport)
#else
#define OFPS_ADDON_EXPORT extern "C" __attribute__((visibility("default")))
#endif

OFPS_ADDON_EXPORT const ofps::sdk::AddonApiV1 *OFPS_CALL PeripheralWarpGetApi(std::uint32_t requestedVersion);
