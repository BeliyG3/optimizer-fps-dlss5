...
using ConsumerCallbackV3 = void (OFPS_CALL *)(const PackedFrameV3 *frame, void *userData);
...
using RegisterConsumerFnV3 = Status (OFPS_CALL *)(const ConsumerRegistrationV3 *, std::uint64_t *);
using GetConfigFnV2 = Status (OFPS_CALL *)(ConfigV2 *);
using SetConfigFnV2 = Status (OFPS_CALL *)(const ConfigV2 *);
using GetLayoutFnV2 = Status (OFPS_CALL *)(std::uint64_t, LayoutV2 *, std::uint64_t *);
using PublishPackedFrameFnV3 = Status (OFPS_CALL *)(const PackedFrameV3 *);
...
OFPS_ADDON_EXPORT const ofps::sdk::AddonApiV3 *OFPS_CALL PeripheralWarpGetApiV3(
