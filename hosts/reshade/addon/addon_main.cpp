...
// lives in DESCRIPTION and in the overlay banner (ofps_version.h, generated from cmake/Version.cmake).
...
void OnPresent(::reshade::api::effect_runtime *)
{
    ...
    static bool eventLogged = false, lateEventLogged = false;
    if (DirectHostActive()) {
        if (!eventLogged) {
            eventLogged = true;
            LogForNgxHook(false, "direct host event detected; shell switched to forward mode");
        }
        if (!lateEventLogged && HasShellFeatures()) {
            lateEventLogged = true;
            LogForNgxHook(true, "direct host appeared after shell feature creation; forwarding existing handles untouched");
        }
    }
    // A failed attachment is latched for this session, outside DllMain's loader lock.
    static const int attachResult = CoreAttach();
    (void)attachResult;
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID reserved)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ofps::reshade::State().module = module;
        DisableThreadLibraryCalls(module);
        if (!::reshade::register_addon(module)) return FALSE;
        ofps::reshade::MigrateLegacyIniOnce();
        ofps::reshade::LoadShellSettings();
        {
            ...
            if (ofps::reshade::CurrentShellSettings().traceExit) ofps::reshade::InstallExitTrace();
            if (ofps::reshade::CurrentShellSettings().debugLayer) {
            ...
            ofps::reshade::LoadDiagnosticsFromReShadeIni();
        }
        {
            ...
            ofps::reshade::CrashGuardInit(path);
        }
        ...
        if (ofps::reshade::CurrentShellSettings().passive) {
            ::reshade::log::message(::reshade::log::level::warning, "Optimizer FPS: Passive=1 - no events, no overlay, no hooks (diagnostics)");
            return TRUE;
        }
        ::reshade::register_event<::reshade::addon_event::init_command_queue>(ofps::reshade::OnInitCommandQueue);
        ::reshade::register_event<::reshade::addon_event::destroy_command_queue>(ofps::reshade::OnDestroyCommandQueue);
        ...
        // Process termination may leave core mutexes owned by terminated threads.
        if (reserved == nullptr) {
            ofps::reshade::ShutdownReadableGuides();
            ofps::reshade::CoreDetach(false);
        }
        ...
        ::reshade::unregister_event<::reshade::addon_event::init_command_queue>(ofps::reshade::OnInitCommandQueue);
        ::reshade::unregister_addon(module);
        ofps::reshade::CloseReShadeOverlayEvent();
    }
    return TRUE;
}
