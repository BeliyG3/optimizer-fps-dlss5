...
// ReShade-independent conversion between ini values and the frozen settings schema.
//
...
// mapping between historical keys and the add-on's settings. ini_store.cpp binds these to
// ReShade, while tests use an in-memory store.
//
...
// The Store selects its own section, so the key alone identifies a value here.
...
struct TemporalConfig { int modelPasses = 1; bool spreadPasses = true; int mode = 0, every = 2, maxQueue = 2; };
inline OfpsSettingsValues SchemaDefaults() {
    OfpsSettingsValues v{}; v.size = sizeof(v); v.count = OFPS_SET_COUNT;
    for (uint32_t i = 0; i < OFPS_SET_COUNT; ++i) v.v[i] = kOfpsSettings[i].defaultValue;
    return v;
}
template<class Store> bool LoadValuesFromStore(const Store &store, OfpsSettingsValues &v) {
    bool any = false;
    for (uint32_t i = 0; i < OFPS_SET_COUNT; ++i) {
        if (i == OFPS_SET_DEBUG_LAYER) continue; // shell_settings owns this early key
        const auto &d = kOfpsSettings[i];
        const bool present = d.type == OFPS_TYPE_FLOAT ? store.GetFloat(d.iniKey, v.v[i].f) : store.GetInt(d.iniKey, v.v[i].i);
        if (present) { v.explicitMask[i / 64] |= 1ull << (i % 64); any = true; }
    }
    return any;
}
template<class Store> void SaveValuesToStore(Store &store, const OfpsSettingsValues &v) {
    for (uint32_t i = 0; i < OFPS_SET_COUNT; ++i) {
        const auto &d = kOfpsSettings[i]; if (!(d.flags & OFPS_FLAG_PERSISTED)) continue;
        if (d.type == OFPS_TYPE_FLOAT) store.SetFloat(d.iniKey, v.v[i].f); else store.SetInt(d.iniKey, v.v[i].i);
    }
}
...
bool LoadConfigFromStore(const Store &store, ofps::sdk::ConfigV2 &config)
{
    bool any = false;
    int integer = 0;
    float number = 0.0f;
    if (store.GetInt("Mode", integer)) {
        config.mode = static_cast<ofps::sdk::WarpMode>(integer); any = true;
    }
    if (store.GetInt("ColorFilter", integer)) {
        config.colorFilter = static_cast<ofps::sdk::ColorFilter>(integer); any = true;
    }
...
template <class Store>
void SaveConfigToStore(Store &store, const ofps::sdk::ConfigV2 &config)
{
...
template <class Store>
void LoadTemporalFromStore(const Store &store, TemporalConfig &temporal)
{
    int integer = 0;
    TemporalConfig t{};
...
template <class Store>
void SaveTemporalToStore(Store &store, const TemporalConfig &temporal)
{
