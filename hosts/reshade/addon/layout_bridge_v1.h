...
/* Call add-on exports on the presenting thread. Status values use the enum above. */
typedef uint32_t (*OptimizerFpsSetSettingV1Fn)(uint32_t id, OfpsSettingValue value);
typedef uint32_t (*PeripheralWarpSetLayoutV1Fn)(const PeripheralWarpLayoutStateV1 *state);
typedef uint32_t (*PeripheralWarpSetTemporalV1Fn)(uint32_t mode, uint32_t every);
#define OFPS_SET_SETTING_V1_EXPORT_NAME "OptimizerFpsSetSettingV1"
#define PW_SET_LAYOUT_V1_EXPORT_NAME "PeripheralWarpSetLayoutV1"
#define PW_SET_TEMPORAL_V1_EXPORT_NAME "PeripheralWarpSetTemporalV1"
