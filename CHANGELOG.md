...
* **Settings beside OptiScaler's menu.** With a public OptiScaler in the process, its menu key
  ...
  OptiScaler's own peripheral compression (`SpatialCompression`) or extra model passes are on.
...
* Background mode runs as the synchronous one when OptiScaler loads ReShade (the game's queue is not
  visible to ReShade).
